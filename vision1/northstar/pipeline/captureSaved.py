...
#                self._video = cv2.VideoCapture("v4l2src device=/dev/video1 ! video/x-raw, width=640, height=480 ! videoconvert ! video/x-raw,format=BGR ! appsink")
               # self._video.set(cv2.CAP_PROP_FRAMEHEIGHT, 600)
               # self._video.set(cv2.CAP_PROP_FRAMEWIDTH, 800)
#				self._video = cv2.VideoCapture("v4l2src device=/dev/video0 ! video/x-raw, width=640, height=480 ! videoconvert ! video/x-raw,format=BGR ! appsink")
